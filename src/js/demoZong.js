...
window.f1 = function() {
    ...
    let timer = setInterval(function(){
        ...
        if(init_width != $('#guzhangcishu')[0].getBBox().width|| times > 50){
            ...
            function cost(obj) {
                // 排序号
                const orderArr = ['Q402','Q502','Q412','Q403','Q314','Q318','Q518','Q161','Q162','Q163','Q165','Q173','Q174','Q175','Q181'];
                ...
                function create() {
                    ...
                    for (let i = 0 ; i <  5 ; i++) {
                        // 如果超出长度则跳出判断
                        const code = orderArr[i + index];
                        if (!code) {
                            break;
                        };
                        // 成本的下标
                        const _index1 = codeArr.findIndex(value => {
                            return value == code;
                        });
                        // 能源的下标
                        const _index2 = energyArr.findIndex(value => {
                            return value[0] == code;
                        });
                        ...
                        // 渲染机组号
                        costCodeShow.eq(i).text(orderArr[i + index]);
                        // 渲染能源
                        if (_index2 !== -1) {
                            let num = 1;
                            let colorIndex = 0;
                            if (energyArr[_index2][2]*1) {
                                num = energyArr[_index2][1]*1 > energyArr[_index2][2]*1 ? 1 : energyArr[_index2][1] / energyArr[_index2][2];
                            };
                            switch (true) {
                                case num>=0.9:
                                    colorIndex = 3;
                            ...
                            text.find('tspan').eq(2).text('/ '+ energyArr[_index2][1]);
                            text.find('tspan').eq(0).text(energyArr[_index2][2]);
                            rect.attr({
                                'width': (251.181 * num),
                        ...
                        if (_index1 !== -1) {
                            g.find('tspan:eq(0)').text(parseInt(costPlanArr[_index1]));
                            g.find('tspan:eq(1)').text( '/'+parseInt(costActArr[_index1]));

                            let num = 0;
                            if (costPlanArr[_index1] == 0 || _index1 == -1) {
                                num = 0;
                            }else {
                                num = ((costActArr[_index1] - costPlanArr[_index1]) / costPlanArr[_index1] * 100).toFixed(0);
                            };
                            if (num > 0) {
                            ...
                            g.find('tspan:eq(0)').text('0');
                            g.find('tspan:eq(1)').text( '/ 0');
                            g.next().find('text').html( '0%');
                        };
                    ...
                    function open ({type,code}) {
                        if (type === 'chengben') {
                            if (code == 'Q318' || code == 'Q518') {
                                window.open('http://10.70.16.168:37799/webroot/decision/view/form?viewlet=硅钢/硅钢三期成本画面318.frm&unit_no='+code);
                            }else {
                            ...
                            switch (code.toUpperCase()) {
                                ...
                                case "Q412":
                                    ...
                                    break;
                                case "Q502":
                                    window.open('http://10.70.16.168:37799/webroot/decision/view/form?viewlet=%E7%A1%85%E9%92%A2/%E7%A1%85%E9%92%A2%E4%B8%89%E6%9C%9F%E6%88%90%E6%9C%AC%E7%94%BB%E9%9D%A2318.frm&unit_no=Q518');
                                    break;
                                case "Q518":
            ...
            function textaling(ele) {

                setTimeout(function () {
                    const width = ele[0].getBBox().width / 2 ;
                    console.log(ele[0].getBBox().width);
                    ele.attr('transform','translate(-'+width+',0)');
            ...
            function addProduct(obj) {
                // 重量
                const product_show = svg.find('#quxiang .text[data-index="1"]');
                const product_no_show = svg.find('#wuquxiang .text[data-index="1"]');
                // 指针
                const pointer_show = svg.find('#quxiang .pointer');
                const pointer_no_show = svg.find('#wuquxiang .pointer');
                // 百分比
                const percentage_num_show = svg.find('#quxiang .text[data-index="0"]');
                const percentage_no_num_show = svg.find('#wuquxiang .text[data-index="0"]');
                // 百分比数据
                ...
                textaling(percentage_num_show.text((num * 100).toFixed(2) + '%'));
                textaling(percentage_no_num_show.text((num_no * 100 ).toFixed(2) + '%'));

                // 绑定指针
                pointer_show.attr('transform','rotate( ' + (num * 180 > 180 ? 180 : num * 180 ) + ' 3373.1641,1037)');
                pointer_no_show.attr('transform','rotate( ' + (num_no * 180 > 180 ? 180 : num_no * 180 ) + ' 3373.1641,1210.169)');
            ...
            function openWindow() {
                ...
                const obj = {
                    ...
                    'QMCPJ401B1518':['#mat_Q518'],
                    'QMCPJ401B1318':['#mat_Q318'],
            ...
            function animation() {
                const circleArr = svg.find('#zonghezhizao path.in,#zonghezhizao path.out');
                let num = 0;
                // 跑马灯
                // 节流阀
                let flag = true;
                ...
                setInterval(function () {
                    ...
                    circleArr.each((index,value) => {
                        ...
                        $circle.attr('transform','rotate('+(num * (1 + index * 0.1))+' 3503.13,410.45)');
            ...
            // start 碳排放
            function tanpaifang() {
                const num = getTd('report1','A36-0').text();

                svg.find('#tanpaifang .text').text(Number(num).toFixed(2));
            }
            tanpaifang();
            // 水印
            function shuiying() {
                const html = `<image href="http://10.70.16.168:37799/webroot/help/svg/img/111.png" x="300" y="230" width="170" height="44"></image><image href="http://10.70.16.168:37799/webroot/help/svg/img/222.png" x="100" y="230" width="170" height="44"></image>`;
                svg.find('#shuiying').html(html);
            }
            // shuiying();
            // end 碳排放
            f2();
